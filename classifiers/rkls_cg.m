function [wb,f,J,obj]=rkls_cg(K,Y,C,varargin);
% Regularised Kernel Logistic Regression Classifier
%
% [alphab,f,J,obj]=rkls_cg(K,Y,C,varargin)
...
opts=struct('alphab',[],'nobias',0,'dim',[],...
            ...
            'maxStep',3,'minStep',5e-2,'marate',.95,'bPC',[],'incThresh',.75,'maxTr',inf);
...
if ( isempty(wb) )    
  ...
  if ( size(K,1)>=1.5*opts.maxTr && all(wb==0) ) % use sub-set to get seed solution
    ...
    wb0 = rkls_cg(K(subidx,subidx),Y(subidx),C.*sum(subidx)/size(K,1)*2,opts); % N.B. re-scale C for less pts
   ...
   wK = wb(1:end-1)'*K;
   % N.B. don't worry about scaling and bias now, as it's dealt with later....
...
wghtY=Y;wght=opts.wght;
wght=opts.wght; wghtY=Y;
if ( ~isempty(wght) ) % point weighting -- only needed in wghtY
...
% precond'd gradient:
%  [K  0  ]^-1 [(lambda*wK-2*K(Y-dv))] = [lambda w - 2*(Y-dv)]
%  [0  bPC]    [ 2*1'*(Y-dv)         ]   [ 2*1'*(Y-dv)./bPC  ] 
MdJ   = [(2*C(1)*wb(1:end-1) - 2*err); ...
         -2*sum(err)./bPC];
...
for iter=1:min(opts.maxIter,2e6);  % stop some matlab versions complaining about index too big
   ...
   for j=1:opts.maxLineSrch;
      ...
      dv    = wK'+(wb(end)+tstep*d(end));
      err   = wght.*(Y-dv);
      % MdJ   = [(2*C(1)*wb(1:end-1) - 2*err); ...
      %          -2*sum(err)./bPC];
      % if ( opts.nobias ) MdJ(end)=0; end
      % dtdJ   =-Kd'*MdJ;  % gradient along the line @ new position
      dtdJ   =-2*(C(1)*(dKw+tstep*dKd) - dK*err + -db*sum(err)); % gradient along the line @ new position
...
% BODGE: find least squares optimal scaling and bias
wK    = (wb(1:end-1)'*K + ridge'.*wb(1:end-1)')./muEig; % include ridge
% find least squares optimal scaling and bias
sb=pinv([C(1)*wK*wb(1:end-1)+wK*(wght'.*wK)' sum(wK*wght); ...
         sum(wK*wght) sum(wght'*single(Y~=0))])*[wK*(wght.*Y); sum(wght'*Y)];
...
[alphab,f,J]=rkls_cg(K,Y,0,'verb',1);

fInds=gennFold(Y,10,'perm',1); 
trnInd=fInds(:,end)<0; tstInd=fInds(:,end)>0; trnSet=find(trnInd);

[alphab,f,J]=rkls_cg(K(trnInd,trnInd),Y(trnInd),1,'verb',1,'ridge',1e-7);
dv=K(tstInd,trnInd)*alphab(1:end-1)+alphab(end);
dv2conf(Y(tstInd),dv)

% test implicit ignored
[alphab0,f0,J0]=rkls_cg(K,Y.*single(fInds(:,end)<0),1,'verb',1,'ridge',1e-7);
