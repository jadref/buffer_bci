...
public class BufferClientClock extends BufferClient {
    ...
    public BufferClientClock(  PApplet parent) {
        super(parent);
        clockSync = new ClockSync(parent);
        System.out.println("bcc made");
    }

    public BufferClientClock(double alpha,   PApplet parent) {
        super(parent);
        clockSync = new ClockSync(alpha, parent);
    }

    public BufferClientClock(ByteOrder order,   PApplet parent) {
        super(order, parent);
        clockSync = new ClockSync(parent);
    }

    public BufferClientClock(ByteOrder order, double alpha,   PApplet parent) {
        super(order, parent);
        clockSync = new ClockSync(alpha, parent);
    }
    ...
    public String getHost() {
        return sockChan.socket().getInetAddress().getHostName();
    }

    public int getPort() {
        return sockChan.socket().getPort();
    }
    ...
    public long getSampOrPoll() throws IOException {
        ...
        if (getSampErr() > maxSampError
                || // error too big
                getTime() > (long) (clockSync.Tlast) + updateInterval
                || // simply too long since we updated
