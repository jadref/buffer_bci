...
public class BufferClient {
  ...
  private String IP_ADDRESS; //This is the ip4 address of the computer running the server
  PApplet parent;
  ...
  public BufferClient(  PApplet parent) {
  ...
  public BufferClient(ByteOrder order, PApplet parent) {
    myOrder = order;
    activeConnection=false;
    autoReconnect=false;
    timeout=500;
  ...
  public synchronized boolean connect(String hostname, int port) throws IOException {
    ...
    sockChan.connect(new InetSocketAddress(InetAddress.getByName(IP_ADDRESS),port));
    activeConnection = sockChan.isConnected();
    if ( activeConnection ) { 
      //System.out.println("active connection");// cache the connection info
  ...
  public boolean setAutoReconnect(boolean val) { 
    autoReconnect=val; 
    return autoReconnect;
  }
  ...
  public synchronized double[][] getDoubleData(int first, int last) throws IOException {
    ...
    //System.out.println("nchans: "+ nChans + ", nSamples: "+ nSamples);
