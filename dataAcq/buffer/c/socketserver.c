...
void *_buffer_socket_func(void *arg) {
	...
	while (SC->keepRunning) {
		...
		if (state >= 2 && canWrite) {
		  /* Note: If the sock has closed before the response is written
			  this can raise a SIGPIPE signal which will crash the buffer! */
		  n = send(sock, curPtr + bytesDone, bytesTotal - bytesDone, 0);//MSG_NOSIGNAL);
			if (n<=0) {
				/* socket was closed */
