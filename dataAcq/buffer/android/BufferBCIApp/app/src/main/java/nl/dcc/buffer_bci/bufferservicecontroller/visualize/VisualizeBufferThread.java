 ...
 */
public class VisualizeBufferThread extends Thread {

    private static final String TAG = VisualizeBufferThread.class.getSimpleName();
    private String host;
    private String feedbackEventType="classifier.prediction";
    private String rawfeedbackEventType="classifier.rawprediction";
    ...
    public VisualizeBufferThread(String host, int port) {
        this.host = host;
        this.port = port;
        C = new BufferClientClock();
    }

    public VisualizeBufferThread(String host, int port, String feedbackEventType) {
        this.host = host;
        this.port = port;
            ...
            if (connect()) {
                ...
                if ( events != null && events.length > 0 ) {
                    ...
                    for (int i = events.length - 1; i >= 0; i--) {
                        ...
                        if (type.equals(feedbackEventType) || type.equals(rawfeedbackEventType) ) {
                            values = convertToFloatArray(events[i].getValue().getArray());
                            if ( type.equals(feedbackEventType) ) damage=true;
                            Log.d(TAG, events[i].toString());
                        }
                    }
                }
            } else {
