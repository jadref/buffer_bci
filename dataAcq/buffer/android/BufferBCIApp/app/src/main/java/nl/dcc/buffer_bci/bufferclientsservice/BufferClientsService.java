...
public class BufferClientsService extends Service {
    ...
    private void broadcastAllThreadInfo(){
            for (int i = 0; i < threadInfos.size(); i++) {
                ...
                threadInfos.valueAt(i).running = threads.get(id).isrunning();
    ...
    private void broadcastThreadInfo(ThreadInfo threadInfo, Argument[] arguments){
        ...
        Log.i(TAG, "Sending broadcast with ThreadID = " + threadInfo.threadID + " running = " + threadInfo.running);
    ...
    private class Updater extends Thread {
        ...
        public void run() {
            while (run) {
                try {
                    Thread.sleep(C.CLIENTS_INFO_UPDATE_INTERVAL);
                    if (update) {
                        broadcastAllThreadInfo();
                    }
                } catch (InterruptedException e) {
