...
public class FBServiceControllerBroadcastReceiver extends BroadcastReceiver {
    ...
    public static String TAG = FBServiceControllerBroadcastReceiver.class.getSimpleName();
    ...
    public void onReceive(final Context context, final Intent intent) {
        if (intent.getIntExtra(C.MESSAGE_TYPE, -1) == C.UPDATE) {
            ...
            if (newBufferInfo) {
                ...
                intent_for_MainActivity.putExtra(C.BUFFER_INFO, bf);
                Log.d(TAG, "From BroadcastReceiver Sending intent with BufferInfo to MainActivity");
