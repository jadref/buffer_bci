...
{  
  public class BufferClientClock : FieldTrip.Buffer.BufferClient {
	 ...
	 public BufferClientClock() : base() {
		clockSync=new ClockSync();
	 }
	 public BufferClientClock(double alpha) : base (){
		clockSync=new ClockSync(alpha);
	 }
	 public BufferClientClock(ByteOrder order) : base(order) {
		clockSync=new ClockSync();
	 }	 
	 public BufferClientClock(ByteOrder order,double alpha) : base(order) {
		clockSync=new ClockSync(alpha);
	 }

	 //--------------------------------------------------------------------
	 // methods offering additional useful functionality
	 public String getHost() { return sockChan.Host; }
	 public int getPort() { return sockChan.Port; }
