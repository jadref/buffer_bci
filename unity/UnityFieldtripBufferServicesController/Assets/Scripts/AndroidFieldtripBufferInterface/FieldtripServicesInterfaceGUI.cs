...
public class FieldtripServicesInterfaceGUI : MonoBehaviour {
		...
		if(androidDevice){
			...
			if(updateClientInfo){
				foreach(int id in clientIDsToUpdateInfo){
					...
					int samplesPut = FieldtripServicesControlerInterface.getClientSamplesPut(id);
					int samplesGotten = FieldtripServicesControlerInterface.getClientSamplesGotten(id);
					int eventsPut = FieldtripServicesControlerInterface.getClientEventsPut(id);
					int eventsGotten = FieldtripServicesControlerInterface.getClientEventsGotten(id);
					...
					string status = FieldtripServicesControlerInterface.getThreadStatus(threadID);
	...
	public void startServer(){
		if(serverSwitch.GetComponent<Toggle>().isOn){
			if(androidDevice){
				serverServiceName = FieldtripServicesControlerInterface.startServer();
	...
	public void stopServer(){
		if(!serverSwitch.GetComponent<Toggle>().isOn){
			if(androidDevice){
				if(FieldtripServicesControlerInterface.stopServer())
	...
	public void startClients(){
		if(clientsSwitch.GetComponent<Toggle>().isOn){
			if(androidDevice){
				clientsServiceName = FieldtripServicesControlerInterface.startClients();
	...
	public void stopClients(){
		if(!clientsSwitch.GetComponent<Toggle>().isOn){
			if(androidDevice){
				if(FieldtripServicesControlerInterface.stopClients())
	...
	private IEnumerator startThread(int threadID, GameObject button){
		if(androidDevice){
			...
			int[] result = FieldtripServicesControlerInterface.getClientIDs();
			for(int i=0; i<result.Length; ++i){
			...
			result = FieldtripServicesControlerInterface.getClientIDs();
			for(int i=0; i<result.Length; ++i){
