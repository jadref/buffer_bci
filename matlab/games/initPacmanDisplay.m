function [arenaax,maph,agentsh,titleax,scoreh,moveh,cmap]=initDisplay(fig,map,agents,key,score,pacmancoords)
  ...
  for y=1:size(map,2);
    switch (map(x,y));
     ...
     case key.pellet;       
      ...
      maph(x,y)=patch(x+xc*rad,y+yc*rad,cmap(key.pellet+1,:),'Linestyle','none','CDataMapping','direct');
     case key.powerpellet;  
      rad=.2; 
      %maph(x,y) =rectangle('position',[x-rad/2 y-rad/2 rad rad],'curvature',[1 1],'EraseMode','background','facecolor',cmap(key.pellet+1,:),'edgecolor',cmap(key.pellet+1,:)); 
      %maph(x,y)=plot(x,y,'.','markerSize',20,'color',cmap(key.powerpellet+1,:));
      maph(x,y)=patch(x+xc*rad,y+yc*rad,cmap(key.powerpellet+1,:),'Linestyle','none','CDataMapping','direct');
...
for i=1:numel(ags);
  ...
  switch agents(x,y);
   case key.pacman;  agentsh(x,y) = patch(x+pacmancoords(1,:)/2*.7,y+pacmancoords(2,:)/2*.7,cmap(key.pacman+1,:),...
                                          'Linestyle','none','CDataMapping','direct');%,'EraseMode','background');
   case key.ghost;   agentsh(x,y) = patch(x+[-1 +1 +1 -1]/2*.7,y+[-1 -1 +1 +1]/2*.7,...
                                          cmap(key.ghost+1+ghosti,:),...
                                          'Linestyle','none','CDataMapping','direct');%,'EraseMode','background');
    ghosti=ghosti+1;
  end
end
