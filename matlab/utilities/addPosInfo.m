function [di]=addPosInfo(di,capFile,overridechnms,prefixMatch,verb,capDir)
...
if ( nargin < 3 ) overridechnms=0; end; % override di's vals info with info from capfile?
...
if ( (isnumeric(vals) || (~isempty(overridechnms) && overridechnms)) )%...
...
for i=1:numel(vals);
   ...
   if ( ~isempty(ti) && numel(ti)==1 && ti>0 && ti<=size(xy,2) ) 
      ...
      di.extra(i).iseeg=true; % default to iseeg channel
		if( any(isnan(di.extra(i).pos2d)) || any(isnan(di.extra(i).pos3d)) ...
			 || any(isinf(di.extra(i).pos2d)) || any(isinf(di.extra(i).pos3d)) ) 
		  di.extra(i).iseeg=false; %pos indicated non-eeg
		end
   else
