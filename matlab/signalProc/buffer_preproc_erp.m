function [X,pipeline]=buffer_preproc_erp(X,hdr,varargin);
% train ERP (time-domain) classifier with ft-buffer based data/events input
%
%   [X,pipeline]=buffer_preproc_erp(X,hdr,varargin);
%
% Inputs:
...
% See Also: preproc_erp, buffer_preproc_ersp
...
[X,pipeline]=preproc_erp(X,'ch_names',ch_names,'ch_pos',ch_pos,'fs',fs,'badCh',~iseeg,varargin{:});
return;
%---------
function testcase();
% buffer stuff
