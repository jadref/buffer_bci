...
if( isempty(saveDir) ) 
   try
      pth=uigetdir(fullfile('~'),'Choose a load/save location.'); 
      fn='';
   catch
     [fn,pth]=uigetdir(fullfile('~'),'Choose a load/save location.'); 
   end
  if ( isequal(pth,0) ) saveDir='.';  % current directory if not given
  else                  saveDir=pth;
  end; 
