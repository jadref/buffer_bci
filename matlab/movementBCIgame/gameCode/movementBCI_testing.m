...
% Computer predictions are made based on muscle and/or brain activity.
% Listens to predictions from cont_apply_classifiers.
% A keyboard needs to be connected to control the game.
% Needs an active buffer and EEG source.
function movementBCI_testing(subject)
            ...
            while ( t_now < trialDuration )
                ...
                if checkKeys %while human has not pressed a button yet
                    ...
                    if ( ~isempty(modekey) ) % if there is a key press
                        ...
                        set(fig,'userdata',[]);
                        sendEvent('humanmove',t_human); % notify the buffer
                ...
                if ~computer_won && ~human_won % while nobody has won yet
                    % check for classifier prediction
                    [events, state] = buffer_newevents(buffhost,buffport,state,'classifier_prediction',[],0);
                     if (~isempty(events)) % classifier 'move' prediction received
                         if t_now > min_time % check if it's not too early
                            t_computer = t_now;
                            sendEvent('predevt_received',1);
                    ...
                    if( t_computer < t_human && t_computer <= t_now ) % computer won
                        ...
                        rt_comp = sendEvent('response.comp',t_computer);
                    ...
                    if( t_human < t_computer && t_human < t_now ) % human won
                        ...
                        sendEvent('response.human', t_human);
