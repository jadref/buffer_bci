function cont_apply_classifiers(subject,hdr,session)
...
% define settings
opts.endType        = 'testing';
opts.endValue       = 'end';
opts.maxFrameLag    = 50;
%opts.predFilt       = @(x,s,samples) percFilt(x,s,300,0.95);
opts.predFilt       = @(x,s,e) biasFilt(x,s,exp(log(0.5)/100)); %@(x,s) biasFilt(x,s,exp(log(.5)/100));
opts.predEventType  = 'classifier_prediction';
opts.step_ms        = 100;
opts.trlen_ms       = 500;
opts.hdr            = hdr;
predictions         = [];
...
% load classifier
try
    load(fullfile('data',[subject,'_comb_classifier']));
    clsfr = clsfr_ERP_ERD;
catch
    msgbox({'Train a classifier first!'},'Error');
end
...
while( ~endTest )
  ...
  for si = 1:numel(fin);    
    ...
    for ci=1:numel(clsfr);
      % preprocess data
      % alpha/beta ERD
      [preproc_ERD,res_ERD,X_ERD,Y_ERD]=buffer_train_ersp_clsfr(data,[],hdr,'fs', hdr.fSample, 'badCh', ~iseeg, 'width_ms', 250,...
      ...
      % RP
      [preproc_ERP,res_RP,X_ERP,Y_ERP]=buffer_train_erp_clsfr(data, [],hdr, 'fs', hdr.fSample, 'badCh', ~iseeg, 'badchrm', 0,...
      ...
      % get prediction
      [f(:,ci),fraw(:,ci)] = applyLinearClassifier(X_ERP_ERD,clsfr_ERP_ERD);
    ...
    if ( isempty(dv) && isempty(opts.predFilt) ) 
      ...
      elseif ( ischar(opts.predFilt) || isa(opts.predFilt,'function_handle') )
        ...
        if dv > 0
            predictions = [predictions; dv fraw fin(si)-trlen_samp]; % only positive predictions, for movement prediction
        end
        all_predictions = [all_predictions; dv fraw fin(si)-trlen_samp];
      end
    end
...
save(fullfile('data',[subject,'_',session,'_predictions']),'predictions');
save(fullfile('data',[subject,'_',session,'_all_predictions']),'all_predictions');
return;
%--------------------------------------
function testCase()
