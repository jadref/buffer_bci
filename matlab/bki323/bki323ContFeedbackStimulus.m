...
score = struct('timetogo',contFeedbackTrialDuration,...
    'pointspossible',maxpointspossible,...
    'curtrialscore',0,...
    'total',0);
progresshdl=text(axLim(1),axLim(2),genTextStr(score),...
...
for si=1:nSeq;
  ...
  score.pointspossible = maxpointspossible;
  score.curtrial = 0;
  firsthit = 0;
  while (timetogo>0)
    ...
    cursorPos=get(h(end),'position'); cursorPos=cursorPos(:);
     ...
     if ( hit ) && ~firsthit %%% update the score...
         firsthit=1;
         score.curtrial = score.curtrial + score.pointspossible;
         set(h(end),'facecolor',tgtColor);
     end
  ...
  score.total = score.total + score.curtrial;
    scorefeedback = sprintf('Trial: %4.0f\nTotal: %4.0f',score.curtrial,score.total);
	 ...
	 set(txthdl,'string',scorefeedback,'color',txtColor,'visible','on');
